...
const STATUS_HE = {
  ...
  never_run:  { label: "לא הופעל", color: "#64748b", emoji: "•" },
};
  ...
  return (
    <div dir="rtl" style={{
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 12 }}>
        <h2 style={{ margin: 0, color: "#0f172a", fontSize: 17 }}>🧠 Stage 0 — ריצת למידה ראשונית</h2>
      ...
      {status && status.status !== "never_run" && (
        <div style={{ background: "#ffffff", borderRadius: 8, padding: 12, fontSize: 12,
                      color: "#0f172a", marginBottom: 14, lineHeight: 1.7 }}>
          ...
          {status.error_message && (
            <div style={{ color: "#dc2626" }}><strong>שגיאה:</strong> {status.error_message}</div>
          )}
        </div>
      )}

      {error && (
        <div style={{ color: "#dc2626", fontSize: 13, marginBottom: 12 }}>שגיאה: {error}</div>
      )}
      ...
      <button
        ...
        style={{
          background: isRunning ? "#64748b" : loading ? "#64748b" : "#3b82f6",
